...
- [x] 1.1 Verify all visible strings in `dashboard/booking/models.py` are wrapped in `_()`.
- [x] 1.2 Verify all visible strings in `dashboard/booking/admin.py` are wrapped in `_()`.
- [x] 1.3 Verify all visible strings in `dashboard/project/settings.py` (UNFOLD settings) are wrapped in `_()`.
...
- [x] 2.2 Wrap environment names in `dashboard/utils/callbacks.py` with `gettext_lazy`.
...
- [x] 3.2 Update `dashboard/locale/es/LC_MESSAGES/django.po` with Spanish translations for all `msgstr ""`.
...
- [x] 4.2 Start the dashboard server and manually verify the admin interface in Spanish.
