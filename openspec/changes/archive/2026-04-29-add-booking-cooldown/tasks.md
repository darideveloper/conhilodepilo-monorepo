...
- [x] 1.1 Add `booking_cooldown_minutes` field to `CompanyProfile` model in `dashboard/booking/models.py` (Default: 0).
- [x] 1.2 Create and apply database migration for the new field.
- [x] 1.3 Add `booking_cooldown_minutes` to `CompanyProfileSerializer` in `dashboard/booking/serializers.py`.

## 2. Dashboard: Availability Logic Refactor & Update
- [x] 2.1 Refactor window-splitting logic in `dashboard/utils/availability.py` into a helper function `_get_free_windows(working_windows, bookings, cooldown)`.
...
- [x] 4.1 Create tests in `dashboard/booking/tests_availability.py` for:
