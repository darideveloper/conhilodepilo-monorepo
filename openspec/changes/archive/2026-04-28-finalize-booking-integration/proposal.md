...
## Why
Currently, the booking flow lacks crucial functionality to be complete. It does not allow users to select specific time slots, collects unnecessary fields (number of persons), lacks a dashboard endpoint to save bookings, and has an incomplete success screen. This change implements the final required integrations.
...
- Add a new `CreateBookingView` POST endpoint in the dashboard.
...
## Impact
- Affected specs: `booking-ui`, `dashboard-models`, `api-contracts`
- Affected code: Frontend components (`BookingCalendar.tsx`, `BookingForm.tsx`), store (`useBookingStore.ts`), API definitions (`booking.ts`), Dashboard models (`models.py`), Dashboard views (`views.py`), and Dashboard tests (`tests_api.py`).
