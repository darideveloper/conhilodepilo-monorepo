## 1. Implementation
- [x] 1.1 Update `get_available_dates` in `backend/utils/availability.py` to accept and process the `30` day timeframe up-front.
...
- [x] 1.4 Modify `_is_day_available` in `backend/utils/availability.py` to accept the pre-fetched structured data instead of the `services` QuerySet.
