...
4. **App Initialization Logic:** Update Astro configurations (`booking` and `landing`) and Django settings (`dashboard`) to explicitly parse `.env` to determine `ENV`, and subsequently load the respective `.env.${ENV}` file.
...
- In `dashboard/project/settings.py`, the existing `dotenv` configuration already partially loads `.env.${ENV}`. We need to ensure `.env.example` represents only `ENV=dev` and that the `.env.dev.example` / `.env.prod.example` templates cover all necessary variables.
