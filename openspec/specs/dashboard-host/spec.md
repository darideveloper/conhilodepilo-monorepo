...
## Purpose
Consolidate the dashboard's base URL configuration into a single `HOST` environment variable. This replaces the redundant and inconsistent use of `SITE_URL`, ensuring a single source of truth for media URL generation, external integration redirects (like Stripe), and security configurations (`ALLOWED_HOSTS`).
## Requirements
