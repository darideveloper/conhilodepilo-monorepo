...
The backend `get_available_dates` calculation MUST gather all necessary date override, availability range, and weekday slot constraints in bulk. The iterations checking daily availability over the 30-day period MUST execute entirely in memory and MUST NOT execute any database queries inside the per-date loop or per-service loop.

#### Scenario: Validating N+1 query elimination
- **Given** the backend has 5 services with various availability slots and overrides.
