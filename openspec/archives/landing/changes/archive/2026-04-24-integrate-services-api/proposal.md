...
## Problem
Currently, `getServices` and `getCourses` return static, hardcoded data with mocked types. The dashboard provides a single endpoint returning a nested array of `ServiceCategory` objects, where the first category represents courses and the subsequent categories represent services.

## Scope
- Update `src/lib/api/types.ts` to reflect the dashboard's `ServiceCategory` and `Service` interfaces.
...
- Configure `astro.config.mjs` to whitelist the dashboard domain for image optimization, reading the domain dynamically from the `PUBLIC_API_URL` environment variable with a fallback to `localhost:8000`.
